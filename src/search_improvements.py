...
def get_adaptive_k_multiplier(query_type: str, base_k: int) -> int:
    ...
    Strategy:
    - how_to: 4x (need more step-by-step guides)
    - comparison: 5x (need multiple viewpoints)
    - factual: 3x (facts are usually clear)
    - definition: 2.5x (definitions are concise)
    - general: 3x (default)
    ...
    multiplier = multipliers.get(query_type, 3)
    # Cap at reasonable limits: minimum 2x, maximum 8x
    raw_k = int(base_k * multiplier)
    return max(base_k * 2, min(raw_k, base_k * 8))
...
def should_enable_hybrid_search(query_type: str, query_length: int) -> bool:
    ...
    # Default: use dense-only for how-to and definition (semantic understanding preferred)
    return False
